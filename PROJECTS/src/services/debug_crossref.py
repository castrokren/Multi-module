...
def check_b_sqlite_db(cfg):
    ...
    if not db_path.exists():
        print("⚠️  No SQLite database found (this is OK if scraper hasn't run yet)")
        return
    ...
    try:
        ...
        try:
            ...
            print("⚠️  Could not query 'downloaded' table")
        ...
        try:
            ...
            seen_count = None
            print("⚠️  Could not query 'seen_urls' table")
        ...
        try:
            ...
            print(f"⚠️  Could not fetch downloaded records: {e}")

        conn.close()

        if downloaded_count == 0:
            print("\n❌ Scraper downloaded zero PDFs!")
        else:
...
def check_c_supplier_matching(cfg):
    ...
    if not excel_files:
        print(f"❌ No Excel files found in {labeled_dir}")
        return
    ...
    try:
        ...
        if not supplier_col:
            print(f"⚠️  Could not find supplier column in Excel")
            return
    ...
    for csv_name in csv_suppliers:
        ...
        if best_score >= threshold:
            ...
            print(f"  ❌ '{csv_name}' -> no match (best: '{best}' {best_score:.2f})")
    ...
    if misses == 0:
        ...
        print("❌ Hit rate < 80% — supplier name mismatch likely!")
...
def check_d_pdf_smart_filter(cfg):
    ...
    try:
        ...
        print(f"❌ Could not import PDFSmartFilter: {e}")
        return
    ...
    if pdf_count > 0:
        ...
        if noise_pct > 80:
            print("❌ Over 80% of PDFs classified as noise!")
        elif noise_pct > 50:
            print("⚠️  Over 50% classified as noise — may be over-filtering")
        else:
