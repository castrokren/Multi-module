...
class Config:
    ...
    def validate(self):
        ...
        if not self.get("output_directory"):
            raise ValueError("output_directory must be set in monitor_config.json")
        output_path = Path(self.get("output_directory"))
        if not output_path.exists():
            raise ValueError(f"output_directory does not exist: {output_path}")
