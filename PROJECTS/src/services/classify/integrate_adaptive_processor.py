...
def migrate_to_adaptive():
    ...
    if not hw_file.exists() or not sw_file.exists():
        print("[ERROR] Keyword files not found. Please ensure hardware_keywords.txt and software_keywords.txt exist.")
        return False
    ...
    try:
        ...
        if len(df_orig) == len(df_adapt):
            ...
            if differences == 0:
                ...
                print("[WARNING]️  Significant differences - review before migrating")
...
def safe_test_adaptive(test_file, min_occurrences=5):
    ...
    if success:
        ...
        print("[ERROR] Test processing failed")
        return False
