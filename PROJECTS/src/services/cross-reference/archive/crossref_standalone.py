...
class CrossReferenceEngine:
    ...
    def run_cross_reference(self, input_file, master_file, pdf_dir, threshold=60):
        ...
        try:
            ...
            if not all([input_file, master_file, pdf_dir]):
                vv_log("[ERROR] Missing required inputs")
                return False
            
            # Validate files exist
            vv_log("Validating file existence...")
            if not os.path.exists(input_file):
                vv_log(f"[ERROR] Input file not found: {input_file}")
                return False
            
            if not os.path.exists(master_file):
                vv_log(f"[ERROR] Master file not found: {master_file}")
                return False
            
            if not os.path.exists(pdf_dir):
                vv_log(f"[ERROR] PDF directory not found: {pdf_dir}")
                return False
            ...
            try:
                ...
                vv_log(f"[ERROR] Failed to load input file: {e}")
                return False
            ...
            try:
                ...
                vv_log(f"[ERROR] Failed to load master file: {e}")
                return False
            ...
            try:
                ...
                if supplier_folders:
                    ...
                    vv_log("[WARNING]️ No supplier folders found")
            except Exception as e:
                vv_log(f"[ERROR] Error listing supplier folders: {e}")
                return False
            ...
            try:
                ...
                vv_log(f"[ERROR] Error counting PDFs: {e}")
                return False
            ...
            for idx, item_row in input_df.iterrows():
                ...
                try:
                    ...
                    if matches:
                        ...
                        vv_log(f"  [ERROR] No matches found")
                
                except Exception as e:
                    vv_log(f"  [ERROR] Error processing item {item_code}: {e}")
                    continue
            ...
            vv_log(f"[ERROR] Cross-reference analysis failed: {e}")
            traceback.print_exc()
            return False
    ...
    def extract_pdf_text(self, pdf_path):
        ...
        try:
            ...
            vv_log("          [ERROR] Could not extract text with either method")
            return ""
        
        except Exception as e:
            vv_log(f"          [ERROR] Error extracting text: {e}")
            return ""
...
def main():
    ...
    if success:
        ...
        if export_success:
            ...
            vv_log("[ERROR] Failed to export results")
    else:
        vv_log("[ERROR] Cross-reference test failed")
