...
def process_single_pdf_safe(args):
    ...
    try:
        ...
        print(f"        [WARNING]️ Error processing {os.path.basename(pdf_path)}: {str(e)[:100]}")
        return None
...
class CrossReferenceRecovery:
    ...
    def run_cross_reference_recovery(self, input_file, master_file, pdf_dir, threshold=60, start_from_item=1):
        ...
        print(f"[RETRY] Recovery mode: Starting from item {start_from_item}")
        ...
        try:
            ...
            print(f"[ERROR] Recovery analysis failed: {e}")
            return False
    ...
    def find_matching_pdfs_safe(self, item_code, description, category, pdf_dir, master_df, threshold, input_df=None):
        ...
        try:
            ...
            if not current_supplier:
                print(f"    [ERROR] No supplier found for item {item_code}")
                return matches
            ...
            for pdf_path, supplier_dir in pdf_files:
                ...
                if processed_count % 10 == 0:
                    print(f"    [CHART] Progress: {processed_count}/{len(pdf_files)} PDFs processed")
                ...
                try:
                    ...
                    print(f"    [WARNING]️ Error processing PDF: {str(e)[:50]}")
                    continue
            
            print(f"    [CHART] Processing completed: {len(matches)} matches found")
            return matches
            
        except Exception as e:
            print(f"    [ERROR] Error in find_matching_pdfs_safe: {e}")
            return matches
...
def main():
    ...
    print("[RETRY] Cross-Reference Recovery Tool")
    print("=" * 50)
