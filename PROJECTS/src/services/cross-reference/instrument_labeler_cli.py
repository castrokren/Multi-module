...
class InstrumentLabelerCLI:
    ...
    def list_items(self, item_type=None, limit=20):
        ...
        print(f"\n[LIST] ITEMS ({len(df_to_show)} total, showing first {min(limit, len(df_to_show))}):")
        print("-" * 80)
    ...
    def change_item_type(self, item_identifier, new_type):
        ...
        if self.cols['code_col']:
            ...
            if self.cols['desc_col']:
                ...
                print("[ERROR] Cannot identify items without code or description column")
                return False
        
        if not mask.any():
            print(f"[ERROR] Item not found: {item_identifier}")
            return False
        
        if mask.sum() > 1:
            print(f"[WARNING]️ Multiple items match '{item_identifier}'. Please be more specific.")
        ...
        if current_type == new_type:
            print(f"[WARNING]️ Item {item_code} is already marked as {new_type}")
            return False
    ...
    def batch_change_by_supplier(self, supplier_name, new_type):
        ...
        if not self.cols['supplier_col']:
            print("[ERROR] No supplier column found")
            return 0
        ...
        if not mask.any():
            print(f"[ERROR] No items found for supplier: {supplier_name}")
            return 0
