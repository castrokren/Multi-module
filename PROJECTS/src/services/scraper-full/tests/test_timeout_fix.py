...
def test_timeout_fixes():
    ...
    print(f"[FOLDER] Input file: {input_file}")
    print(f"[FOLDER] Master file: {master_file}")
    print(f"[FOLDER] PDF directory: {pdf_dir}")
    ...
    if not os.path.exists(pdf_dir):
        ...
        try:
            ...
            print(f"[ERROR] Failed to create test PDFs: {e}")
            return
    ...
    try:
        ...
        if not pdf_files and not supplier_dirs:
            print(f"[ERROR] No PDF files or supplier directories found in: {pdf_dir}")
        ...
        print(f"[ERROR] Error reading PDF directory: {e}")
        return
    
    try:
        # Run the analysis with timeout protection
        print("\n[ROCKET] Starting test analysis with timeout protection...")
        ...
        if success:
            ...
            if export_success:
                ...
                print("[WARNING]️ Analysis completed but export failed")
        else:
