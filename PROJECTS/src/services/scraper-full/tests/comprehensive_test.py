    ...
    try:
        ...
        if os.path.exists(input_file):
            try:
                ...
                debug_log(f"[CHART] First few item codes: {df['TYPE'].head(5).tolist() if 'TYPE' in df.columns else 'No TYPE column'}")
            except Exception as e:
                debug_log(f"[ERROR] Error reading input file: {e}")
        else:
        ...
        if os.path.exists(master_file):
            try:
                ...
                debug_log(f"[ERROR] Error reading master file: {e}")
        else:
            debug_log(f"[ERROR] Master file not found: {master_file}")
            return
        ...
        if os.path.exists(pdf_dir):
            try:
                ...
                for supplier in supplier_dirs[:5]:  # Check first 5
                    ...
                    debug_log(f"  [FILE] {supplier}: {len(supplier_pdfs)} PDFs")
                
                debug_log(f"[CHART] Total PDFs in first 5 suppliers: {total_pdfs}")
                
            except Exception as e:
                debug_log(f"[ERROR] Error reading PDF directory: {e}")
        else:
            debug_log(f"[ERROR] PDF directory not found: {pdf_dir}")
            return
        ...
        if success:
            ...
            if hasattr(engine, 'results') and engine.results:
                debug_log(f"[CHART] Found {len(engine.results)} matches!")
                ...
                debug_log("[CHART] No matches found in results")
            
            # Check for output files
            result_files = [f for f in os.listdir('.') if f.startswith('crossref_results_')]
            if result_files:
                latest_result = max(result_files)
                debug_log(f"[FOLDER] Results file created: {latest_result}")
            else:
                debug_log("[FOLDER] No results file created")
        else:
