...
def main():
    ...
    try:
        ...
        print(f"[ROCKET] Starting reasonable limits test...")
        print(f"[CHART] Will process up to 100 items from Excel file")
        print(f"[CHART] Will run for up to 30 minutes maximum")
        ...
        if success:
            ...
            if result_files:
                ...
                print(f"[CHART] Results saved to: {latest_result}")
            else:
                print("[CHART] No results file found - may indicate no matches")
        else:
            print("[WARNING]️ Test completed but analysis failed")
            
        # Set stop flag to prevent timeout
        stop_flag['value'] = True
        
    except Exception as e:
        print(f"[ERROR] Reasonable limits test error: {e}")
        import traceback
        traceback.print_exc()
        stop_flag['value'] = True
    finally:
