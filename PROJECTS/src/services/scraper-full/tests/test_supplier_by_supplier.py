...
def main():
    ...
    print("[LIST] This approach processes one supplier directory at a time")
    ...
    try:
        ...
        print(f"[ROCKET] Starting supplier-by-supplier test...")
        ...
        if success:
            ...
            if hasattr(engine, 'results') and engine.results:
                print(f"[CHART] Found {len(engine.results)} total matches!")
        ...
        print(f"[ERROR] Supplier-by-supplier test error: {e}")
        import traceback
        traceback.print_exc()
        stop_flag['value'] = True
    finally:
