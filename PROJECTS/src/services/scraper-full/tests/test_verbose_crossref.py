...
def main():
    ...
    for name, path in test_files.items():
        ...
        status = "✅ EXISTS" if exists else "[ERROR] MISSING"
    ...
    if os.path.exists("crossref_standalone.py"):
        ...
        print("[ERROR] crossref_standalone.py not found")
        return
    ...
    try:
        ...
        if not all(os.path.exists(path) for path in test_files.values()):
            print("[ERROR] Missing required files. Please ensure:")
        ...
        print(f"Success: {'✅ YES' if success else '[ERROR] NO'}")
