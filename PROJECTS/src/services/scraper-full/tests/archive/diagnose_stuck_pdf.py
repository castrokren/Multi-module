        ...
        try:
            if proc.info['name'] == 'python.exe':
                ...
                if 'crossref' in cmdline.lower():
                    print(f"[RETRY] Killing stuck process: PID {proc.info['pid']}")
                    proc.terminate()
                    killed_count += 1
...
def check_pdf_file(pdf_path):
    ...
    print(f"[FILE] PDF file size: {file_size / (1024*1024):.2f} MB")
    
    if file_size > 50 * 1024 * 1024:  # 50MB
        print("[WARNING]️ PDF is very large (>50MB) - may cause timeouts")
        return False
...
def test_pdf_extraction(pdf_path, timeout=10):
    ...
    try:
        ...
        try:
            ...
            print(f"[WARNING]️ PyPDF2 failed: {e}")
        ...
        print("[ERROR] Both extraction methods failed")
        return False
...
def find_problematic_pdf(pdf_dir, start_index=2070, end_index=2074):
    ...
    if start_index <= len(all_pdfs) <= end_index:
        print(f"[WARNING]️ PDF index {start_index} is out of range (total: {len(all_pdfs)})")
        return None
    ...
    if start_index < len(all_pdfs):
        ...
        if check_pdf_file(pdf_path):
            if test_pdf_extraction(pdf_path):
                ...
                print("[ERROR] PDF extraction failed - this might be the problematic file")
                return pdf_path
    else:
        print(f"[ERROR] PDF index {start_index} not found")
    
    return None

def create_safe_pdf_list(pdf_dir, exclude_problematic=True):
    """Create a list of safe PDFs to process."""
    print("[LIST] Creating safe PDF list...")
    ...
    print(f"[WARNING]️ Problematic PDFs: {len(problematic_pdfs)}")
