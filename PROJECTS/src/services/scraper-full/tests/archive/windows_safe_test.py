...
def main():
    ...
    try:
        ...
        print(f"[ROCKET] Starting Windows-safe test with timeout protection...")
        ...
        if success:
            ...
            print("[WARNING]️ Test completed (no matches found - normal for test data)")
            
        # Set stop flag to prevent timeout
        stop_flag['value'] = True
        
    except Exception as e:
        print(f"[ERROR] Windows-safe test error: {e}")
        import traceback
        traceback.print_exc()
        stop_flag['value'] = True
    finally:
