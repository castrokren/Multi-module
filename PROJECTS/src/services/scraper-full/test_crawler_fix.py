    ...
    for batch_start in range(0, len(suppliers), batch_size):
        ...
        def mock_crawl_vendor(supplier, url):
            try:
                ...
                print(f"  [ERROR] Error crawling {supplier}: {e}")
            finally:
                semaphore.release()
        ...
        for thread in batch_threads:
            ...
            if thread.is_alive():
                print(f"  [WARNING]️ Thread timeout - some suppliers in batch may still be processing")
...
def test_visited_set_isolation():
    ...
    for supplier_id, page_count in results.items():
        if page_count == 3:
            ...
            print(f"  [ERROR] {supplier_id}: expected 3 pages, got {page_count}")
