...
def debug_single_instance():
    ...
    try:
        ...
        print(f"[ERROR] Single instance check failed: {e}")
...
def main():
    ...
    if not debug_imports():
        print("[ERROR] Import test failed - check your Python environment")
        return
    
    if not debug_single_instance():
        print("[ERROR] Single instance test failed - try closing any running instances")
        return
    
    if not debug_gui():
        print("[ERROR] GUI test failed - check your display/X11 setup")
        return
    
    if not debug_main_import():
        print("[ERROR] Main module test failed - check for syntax errors")
        return
    ...
    try:
        ...
        print(f"[ERROR] Failed to start scraper: {e}")
        traceback.print_exc()
